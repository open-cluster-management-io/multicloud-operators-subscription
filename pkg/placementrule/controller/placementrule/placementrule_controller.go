...
func add(mgr manager.Manager, r reconcile.Reconciler) error {
	...
	placementRulePred := predicate.Funcs{
		CreateFunc: func(e event.CreateEvent) bool {
			// no work if placementrule has apps.open-cluster-management.io/experimental-controller-disable: "true" annotation
			if value, ok := e.Object.GetAnnotations()[appv1alpha1.PlacementRuleDisableAnnotationkey]; ok && value == "true" {
				return false
			}
			return true
		},
		UpdateFunc: func(e event.UpdateEvent) bool {
			if value, ok := e.ObjectNew.GetAnnotations()[appv1alpha1.PlacementRuleDisableAnnotationkey]; ok && value == "true" {
				return false
			}
	...
	err = c.Watch(&source.Kind{Type: &appv1alpha1.PlacementRule{}}, &handler.EnqueueRequestForObject{}, placementRulePred)
	if err != nil {
		return err
	}
